...
description: Guided initial installation of the fable framework family into a target project — inspect the target first, interview for a configuration, enforce the exclusivity rules, assemble CLAUDE.md / AGENTS.md / skills / hooks in the correct order with idempotent signature checks, then verify with agent-framework-doctor. Use when asked to install, set up, or add the agent framework (the fable family, or any of its frameworks/modules) to a new or existing project.
---

# agent-framework-setup — install the family without breaking the target

This skill runs in the Fable-Agent-Framework store and writes into a target project. `frameworks/` is the source of truth; the root README carries the same recipes for humans — this skill adds what raw snippets cannot: inspection before writing, merging instead of overwriting, and verification at the end.
...
Inventory installed fable components: every template announces itself with an HTML comment `<!-- fable-<name> vX.Y` in its first lines — grep CLAUDE.md and AGENTS.md for `<!-- fable-`. Three modes fall out:

- **Fresh** — no fable signatures anywhere.
...
- solo inlines all protocols → do not install any fable skills alongside it.
...
|---|---|---|---|
| solo | base ← `fable-solo/CLAUDE.template.md` | — (inlined) | — |
| lift | base ← `fable-lift/CLAUDE.template.md` | deep-plan, finish-gate, long-task-state, root-cause-debug, test-protocol | — |
...
| team | bridge line `@AGENTS.md` near the top | — | `AGENTS.md` ← `fable-team/AGENTS.template.md` |
...
- **Idempotency**: before appending a component, grep the target file for its `<!-- fable-<name>` signature; if present, skip it and say so in the report. Upgrading an older installed version in place is manual work, not this skill's.
- **Existing CLAUDE.md** (fresh install over a real project): the fable base becomes the file; the previous content moves under its `## Project specifics` section. Keep the original as `CLAUDE.md.bak` until agent-framework-doctor passes, then delete it.
