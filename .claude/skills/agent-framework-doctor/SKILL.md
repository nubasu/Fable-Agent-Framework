...
| C3 | Skills complete and well-placed: for each installed module its skills exist at `<proj>/.claude/skills/<name>/SKILL.md` with frontmatter `name:` equal to the directory name. Expected sets — lift: deep-plan, finish-gate, long-task-state, root-cause-debug, test-protocol · orchestrate: delegate, accept-work · blueprint: spec-interrogate, design-doc, ticketize · retro: retro, session-bootstrap · incident: incident-response, postmortem. Extra non-pseudo-fable skills are the project's own business. | missing / mismatched → FAIL |
...
| C5 | AGENTS.md variant identified: team (`<!-- pseudo-fable-team`) XOR orchestrate-minimal (`<!-- pseudo-fable-orchestrate`). team without an `@AGENTS.md` bridge line in CLAUDE.md works only where Claude Code reads AGENTS.md natively. | both signatures → FAIL; missing bridge → WARN |
