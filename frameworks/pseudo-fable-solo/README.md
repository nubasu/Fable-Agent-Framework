...
## vs. pseudo-fable-lift (both uplift a single agent)

| | pseudo-fable-lift | pseudo-fable-solo |
|---|---|---|
...
- **pseudo-fable-solo** — solo session, one sheet, full depth (this framework)
- **pseudo-fable-lift** — two-layer single-agent uplift (Opus/Sonnet, general-purpose)
- **pseudo-fable-team** — distilled one-sheet for mixed teams (PL + workers) (AGENTS.md)
- **pseudo-fable-blueprint / pseudo-fable-orchestrate** — upstream work / delegation & acceptance (combined with solo/lift when running a team)
