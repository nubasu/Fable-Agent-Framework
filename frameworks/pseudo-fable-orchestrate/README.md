...
Sister framework of `pseudo-fable-lift`. Division of labor:

- **pseudo-fable-blueprint** — discipline for turning a spec into design, plan, and tickets (upstream; its tickets map 1:1 to this framework's briefs)
- **pseudo-fable-lift** — discipline for your own hands (verification, debugging, reporting)
- **pseudo-fable-orchestrate** — discipline for getting work done through others (agents): delegation, acceptance, integration

Install both in the lead session. It works without pseudo-fable-lift, but the references to `deep-plan` and `long-task-state` pay off fully when combined.

There is also `frameworks/pseudo-fable-team/`, a team constitution (AGENTS.md form) that distills the whole family into one role-dispatching sheet. Since the repo root can hold only one AGENTS.md: use the pseudo-fable-team version to run a mixed team (PL + workers) off a single sheet, or this framework's bundled minimal `AGENTS.template.md` when the lead context lives entirely in CLAUDE.md and you only need ground rules for external workers (the pseudo-fable-team version is a superset of the minimal one).
...
- **What doesn't**: the subagents' own capability (that's the job of pseudo-fable-lift / AGENTS.md on each agent's side). Nor does it make inherently undelegatable work (vague requirements, cross-cutting design) delegatable — the routing table exists largely to say "don't delegate this".
...
The aim is leverage: reinvest the tokens not spent on implementation into speccing and verification (thicker Gotchas, contracts hardened as tests, deeper acceptance, more parallel streams). To uplift the implementation side, we strongly recommend putting pseudo-fable-lift into the CLAUDE.md that workers inherit (Claude subagents read the project's CLAUDE.md).

## Operating rules (growing it)

- Same as pseudo-fable-lift: **add rules by working backward from recurring failures; delete rules that never fire.**
...
- Text-based discipline is strong steering, not enforcement (same as pseudo-fable-lift). Mechanical reinforcement of acceptance (a PostToolUse nudge after every subagent return) is available in the optional `frameworks/pseudo-fable-harness/` module.
