...
Family pipeline: spec → pseudo-fable-blueprint (design, plan, tickets) → pseudo-fable-orchestrate (delegation, acceptance) → pseudo-fable-lift (execution discipline). The one-sheet options are pseudo-fable-team (mixed-team distillation) and pseudo-fable-solo (solo Opus, full depth). pseudo-fable-retro (session restore & rule cultivation), pseudo-fable-incident (incident response), and pseudo-fable-harness (hook-based mechanical guardrails) can each be added to any configuration.
